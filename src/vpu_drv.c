...
#include "include/vpu_drv.h"
#include "include/vpu_log.h"
#include "include/vpu_list.h"
...
RK_S32 vpu_power_on(VPU_INFO_S *p)
{
    if (!p->power_on)
    {
        VPM_DEBUG("VPU: vpu_power_on %d\n", p->power_on);
        if (ioctl(p->fd_vpu, VPU_IOC_CLOCK_ON))
        {
            VPM_ERROR("VPU: VPU_IOC_POWER_ON failed\n");
            return VPU_FAILURE;
        }
    }
    p->power_on = 1;
...
RK_S32 vpu_power_off(VPU_INFO_S *p)
{
    if (p->power_on)
    {
        VPM_DEBUG("VPU: vpu_power_off %d\n", p->power_on);
        if (ioctl(p->fd_vpu, VPU_IOC_CLOCK_OFF))
        {
            VPM_ERROR("VPU: VPU_IOC_POWER_OFF failed\n");
            return VPU_FAILURE;
        }
    }
    p->power_on = 0;
...
RK_S32 vpu_get_enc_info(VPU_INFO_S *p)
{
    ...
    pHwCfg->busType = vpu_service_check_hw(pRegs);
    pHwCfg->busWidth = 0;
    pHwCfg->synthesisLanguage = 0;
    //pHwCfg->reserv[0] = pHwCfg->reserv[1] = 0;


    VPM_INFO("EWLReadAsicConfig:\n"
          "    maxEncodedWidth   = %d\n"
          "    h264Enabled       = %s\n"
          "    jpegEnabled       = %s\n"
          "    mpeg4Enabled      = %s\n"
...
RK_S32 vpu_mmap_and_get_info(VPU_INFO_S *p, int fd)
{
    ...
    VPM_INFO("VPU: ioctl get offset done\n");
...
#if 0
    {
        ...
        {
            VPM_ERROR("VPU: failed to mmap regs\n");
            VPU_ERROR(-4);
        }
        else
        {
    ...
    VPM_INFO("VPU: VPURegMap : 0x%.08x mmap done\n", (RK_U32)p->VPURegMap);
#endif

    if (err = vpu_power_on(p))
    {
        VPM_ERROR("VPU: vpu_power_on failed ret %d\n", err);
        VPU_ERROR(-1);
    }

    if (err = ioctl(p->fd_vpu, VPU_IOC_RD_ENC, (RK_U32)&p->VPURegEnc[0]))
    {
        VPM_ERROR("VPU: VPU_IOC_RD_ENC failed \n");
        VPU_ERROR(-5);
    }
    ...
    VPM_INFO("VPU: vpu_get_dec_info done\n");

    vpu_get_enc_info(p);

    VPM_INFO("VPU: vpu_get_enc_info done\n");
...
RK_S32 vpu_try_accept_task(VPU_INFO_S *p, VPU_TASK_S *pTask)
{
    ...
    {
        VPM_INFO("VPU: vpu_try_accept_task 0x%.8x failed\n", pTask);
        return VPU_FAILURE;
    }
    else
    {
        ...
        {
            p->codec = pTask;
            VPM_INFO("VPU: vpu_try_accept_task codec 0x%.8x success\n", pTask);
        }
...
RK_S32 vpu_release_task(VPU_INFO_S *p, VPU_TASK_S *pTask)
{
    if (USE_CODEC(pTask)) {
        if (ioctl(p->fd_vpu, VPU_IOC_CLS_IRQ, pTask->client_type)) {
            VPM_ERROR("VPU: VPU_IOC_CLS_IRQ failed\n");
        }
        p->codec = NULL;
        p->resev = NULL;
        VPM_INFO("VPU: vpu_release_task codec\n");
    }
    ...
    if (USE_PPROC(pTask)) {
        ...
        p->pproc = NULL;
        VPM_INFO("VPU: vpu_release_task pproc\n");
    }
...
RK_S32 vpu_connect(VPU_INFO_S *p, int sock)
{
    ...
    VPM_INFO("VPU: accept client %d\n", sock);

    if (p->fds_used >= VPU_FD_TASK_NUM) {
        VPM_ERROR("VPU: reach max connection number, refuse socket: %d\n", sock);
        goto SOCK_REFUSE;
    }

    {
        ...
        if (poll(&fd, 1, -1) <= 0) {
            VPM_ERROR("VPU: poll client socket %d fail\n", sock);
            goto SOCK_REFUSE;
        }

        if (!(fd.revents & POLLIN)) {
            VPM_ERROR("VPU: poll client socket %d unexpected return\n", sock);
            goto SOCK_REFUSE;
        }
    }
    ...
    if (len != sizeof(VPU_REGISTER_S)) {
        VPM_ERROR("VPU: client socket %d recv msg failed len %d\n", sock, len);
        goto SOCK_REFUSE;
    }

    if (VPU_CMD_REGISTER != cmd) {
        VPM_ERROR("VPU: client socket %d register cmd err: %d\n", sock, cmd);
        goto SOCK_REFUSE;
    }

    if (msg->client_type >= VPU_TYPE_BUTT) {
        VPM_ERROR("VPU: client socket %d err client type: %d\n", sock, msg->client_type);
        goto SOCK_REFUSE;
    }

    pTask = malloc(sizeof(VPU_TASK_S));
    if (NULL == pTask) {
        VPM_ERROR("VPU: accept client socket %d malloc fail\n", sock);
        goto SOCK_REFUSE;
    }

    if (VPUSendMsg(sock, VPU_CMD_REGISTER_ACK_OK, NULL, 0, 0))
    {
        VPM_ERROR("VPU: send VPU_CMD_REGISTER_ACK_OK faild\n");
        goto SOCK_REFUSE;
    }

    if (vpu_power_on(p))
    {
        VPM_ERROR("VPU: vpu_power_on faild\n");
        goto SOCK_REFUSE;
    }
    ...
    for (i = VPU_TASK_ID_MIN; i < VPU_TASK_ID_MAX; i++)
    {
        if (-1 == fds[i].fd)
        {
            VPM_INFO("VPU: find fd %d for store sock %d\n", i, sock);
            break;
        }
    }
    ...
    p->fds_used++;
    VPM_INFO("VPU: accept client %d id %d success total %d task\n", sock, pTask->fds_id, p->fds_used);
    return 0;
...
RK_S32 vpu_isr(VPU_INFO_S *p, RK_U32 event)
{
    ...
    if ((NULL == p) || ((NULL == p->codec) && (NULL == p->pproc))) {
        VPM_DEBUG("VPU: no task wait for interrupt!!\n");
        return -1;
    }

    if (event & VPU_IRQ_EVENT_ENC_BIT) {
        if (NULL == p->codec) {
            VPM_DEBUG("VPU: no enc task wait for enc irq\n");
            return -2;
        } else {
            VPU_TASK_S *pTask   = p->codec;
            if (ioctl(p->fd_vpu, VPU_IOC_RD_ENC, (RK_U32)p->VPURegEnc)) {
                VPM_ERROR("VPU: VPU_IOC_WR_ENC failed\n");
                return -3;
            }

            if (VPUSendMsg(pTask->sock, VPU_SEND_CONFIG_ACK_OK, p->VPURegEnc, SIZE_REG(VPU_REG_NUM_ENC), 0)) {
                VPM_ERROR("VPU: send VPU_ENC_SEND_CONFIG_ACK_OK faild\n");
            } else {
                VPM_INFO("VPU: send VPU_ENC_SEND_CONFIG_ACK_OK success\n");
            }
            vpu_release_task(p, pTask);
        }
    }
    ...
    if (event & VPU_IRQ_EVENT_DEC_BIT) {
        if (!((event & VPU_IRQ_EVENT_DEC_IRQ_BIT) || (event & VPU_IRQ_EVENT_PP_IRQ_BIT))) {
            VPM_ERROR("VPU_IRQ_EVENT_DEC but no IRQ_BIT!");
            return -4;
        }

        if (event & VPU_IRQ_EVENT_DEC_IRQ_BIT) {
            if (NULL == p->codec) {
                VPM_DEBUG("VPU: no dec task wait for dec irq\n");
                return -5;
            } else {
                VPU_TASK_S *pTask   = p->codec;
                RK_U32  irq_status;
                ...
                if (VPU_DEC == pTask->client_type) {
                    ...
                    VPM_ERROR("VPU: errer task type for dec irq\n");
                    return -6;
                }

                if (ioctl(p->fd_vpu, io_type, (RK_U32)p->VPURegDec)) {
                    VPM_ERROR("VPU: VPU_IOC_RD_DEC failed\n");
                    return -7;
                }
                ...
                if (!(irq_status&IRQ_DEC_ERROR) && (irq_status&IRQ_DEC_BUFFER_EMPTY)) {
                    p->resev = pTask;
                    VPM_INFO("VPU: dec task reserve success 0x%.8x\n", pTask);
                } else {
                    vpu_release_task(p, pTask);
                }
            }
    ...
    if (event & VPU_IRQ_EVENT_DEC_BIT)
    {
        ...
        if (event & VPU_IRQ_EVENT_DEC_IRQ_BIT)
        {
            if (NULL == p->codec) {
                ...
                VPM_DEBUG("VPU: irq_status 0x%.8x\n", p->VPURegMap[pTask->reg_irqs]);

                assert(VPU_ENC != pTask->client_type);

                if (VPUSendMsg(pTask->sock, VPU_SEND_CONFIG_ACK_OK, reg_base, reg_size, 0))
                {
                    VPM_ERROR("VPU: send dec VPU_SEND_CONFIG_ACK_DONE faild\n");
                }
                else
                {
                    VPM_INFO("VPU: send dec VPU_SEND_CONFIG_ACK_DONE success\n");
                }
        ...
        if (event & VPU_IRQ_EVENT_PP_IRQ_BIT)
        {
            if (NULL == p->pproc) {
                ...
                if (VPUSendMsg(pTask->sock, VPU_SEND_CONFIG_ACK_OK, reg_base, reg_size, 0))
                {
                    VPM_ERROR("VPU: send pp VPU_SEND_CONFIG_ACK_DONE faild\n");
                }
                else
                {
                    VPM_INFO("VPU: send pp VPU_SEND_CONFIG_ACK_DONE success\n");
                }
...
RK_S32 VPUSendMsg(int sock, VPU_CMD_TYPE cmd, void *msg, RK_U32 len, int flags)
{
    ...
    VPM_INFO("VPUSendMsg : sock : %8d msg : 0x%.8x cmd : %4d len : %4d\n", sock, (RK_U32)msg, cmd, len);
...
RK_S32 VPURecvMsg(int sock, VPU_CMD_TYPE *cmd, void *msg, RK_U32 len, int flags)
{
    ...
    VPM_INFO("VPURecvMsg : sock : %8d msg : 0x%.8x\n", sock, (RK_U32)msg);
...
{
    VPM_INFO("VPU: VPUProcMsg %d len %d\n", cmd, len);
    ...
    case VPU_SEND_CONFIG :
        VPM_INFO("VPU: VPUProcMsg VPU_SEND_CONFIG\n");
        ...
        //if ((RK_U32)len == (pTask->reg_size * sizeof(RK_U32)))
        {
            ...
            case VPU_ENC : {
                VPM_INFO("VPU: VPUProcMsg VPU_ENC_SEND_CONFIG\n");
/*
...
*/
            ...
            case VPU_DEC : {
                VPM_INFO("VPU: VPUProcMsg VPU_DEC_SEND_CONFIG\n");
/*
...
*/
                ...
                {
                    VPM_ERROR("VPU: VPU_IOC_WR_DEC failed\n");
                    return VPU_FAILURE;
                }
                break;
            }
            case VPU_PP : {
                VPM_INFO("VPU: VPUProcMsg VPU_PP_SEND_CONFIG\n");
/*
                //    preg[i] = pMsg[i];
...
*/
                ...
                {
                    VPM_ERROR("VPU: VPU_IOC_WR_PP failed\n");
                    return VPU_FAILURE;
                }
                break;
            }
            case VPU_DEC_PP : {
                VPM_INFO("VPU: VPUProcMsg VPU_DEC_PP_SEND_CONFIG\n");
/*
...
*/
                ...
                {
                    VPM_ERROR("VPU: VPU_IOC_WR_DEC_PP failed\n");
                    return VPU_FAILURE;
                }
                break;
            }
            default : {
                VPM_ERROR("VPU: VPUProcMsg VPU_DEC_PP_SEND_CONFIG error\n");
                break;
            }
            }
        }
        ...
        else
        {
            ...
            {
                VPM_ERROR("VPU: send VPU_SEND_CONFIG_ACK_FAIL faild\n");
            }
            vpu_release_task(p, pTask);
        }
#endif
    ...
    case VPU_CMD_REGISTER :
        VPM_ERROR("VPU: one task can not register more than once!!\n");
        break;

    default :
        VPM_ERROR("undefined command\n");
        break;
    }

    VPM_INFO("VPU: VPUProcMsg %d len %d done\n", cmd, len);
...
void *vpu_service(void *arg)
{
    ...
    {
        VPM_INFO("vpu_service exsists, use kernel function\n");
        return NULL;
    }
    ...
    if (NULL == p)
    {
        VPM_ERROR("VPU: failed to malloc vpu info\n");
        VPU_ERROR(-1);
    }
    ...
    if (fd == -1)
    {
        VPM_ERROR("VPU: failed to open vpu\n");
        VPU_ERROR(-1);
    }

    VPM_INFO("VPU: vpu opened\n");

    /* mmap hardware device register to virtual address */
    vpu_mmap_and_get_info(p, fd);

    VPM_INFO("VPU: service started\n");

    {
        ...
        if (sock < 0)
        {
            VPM_ERROR("VPU: failed to create socket\n");
            VPU_ERROR(-2);
        }

        VPM_INFO("VPU: socket create done\n");


        if (listen(sock, 10) == -1)
        {
            VPM_ERROR("VPU: listen: %sock", strerror(errno));
            VPU_ERROR(-4);
        }

        VPM_INFO("VPU: server start recving\n");
        ...
        VPM_INFO("VPU: doing message process\n");

        for (;;)
        {
            if (poll(fds, VPU_FD_MAX_NUM, -1) > 0)
            {
                VPM_INFO("VPU: poll return\n");
                ...
                {
                    RK_U32 event;
                    VPM_INFO("VPU: start to read interrupt event\n");

                    if (read(fds[VPU_FD_ISR].fd, &event, sizeof(event)) == 4)
                    {
                        VPM_INFO("VPU: irq read 0x%x\n", event);
                        // 返回的是中断的数量
                        vpu_isr(p, event);
                    }
                    else
                    {
                        VPM_ERROR("VPU: read failed\n");
                    }
                }
                ...
                if (fds[VPU_FD_CONNECT].revents & POLLIN)
                {
                    ...
                    if (client_sock >= 0) {
                        ...
                        VPM_ERROR("VPU: accept client %d failed\n", client_sock);
                    }
                }
                    ...
                    list_for_each_safe(list, tmp, &p->fds_list.list)
                    {
                                ...
                                (VPU_SUCCESS == vpu_try_accept_task(p, pTask)))
                        {
                            ...
                            if (fds[pTask->fds_id].revents == POLLIN)
                            {
                                ...
                                RK_S32 len = 0;
                                VPU_CMD_TYPE cmd;
                                VPM_INFO("VPU: reading sock %d\n", sock);
                                len = VPURecvMsg(sock, &cmd, (void *)p->msg, sizeof(p->msg), 0);

                                if (len >= 0)
                                {
                                    if (VPU_SUCCESS == VPUProcMsg(p, pTask, cmd, len))
                                        VPM_INFO("VPU: task proc msg success\n");
                                    else
                                    {
                                        release_task = 1;
                                        VPM_ERROR("VPU: task proc msg failed, len = %d\n", len);
                                    }
                                }
                                else
                                {
                                    release_task = 1;
                                    VPM_ERROR("VPU: VPURecvMsg failed\n");
                                }
                            }
                            else
                            {
                                //release_task = 0;
                                VPM_DEBUG("VPU: task: type %d id %d poll return not only pollin\n", pTask->client_type, pTask->fds_id);
                                // 防止通道关闭
                                ...
                                if (fds[pTask->fds_id].fd != -1)
                                {
                                    ...
                                    {
                                        // 错误处理
                                        VPM_DEBUG("VPU: poll socket revents 0x%x\n", fds[pTask->fds_id].revents);
                                    }
                                }
                                else
                                {
                                    VPM_DEBUG("VPU: task %.8x id %d socket is released\n", (RK_U32)pTask, pTask->fds_id);
                                }
                        ...
                        {
                            VPM_INFO("VPU: skip task: type %d id %d events %d\n", pTask->client_type, pTask->fds_id, fds[pTask->fds_id].revents);
                        }
                    }

                    VPM_INFO("VPU: list_for_each_safe done\n");
                }
            }
            else
            {
                VPM_ERROR("poll failed %d\n", errno);
            }
        }

        VPM_ERROR("msg process done\n");

        shutdown(sock, SHUT_RDWR);
        close(sock);
        VPM_ERROR("Server done\n");
    }
