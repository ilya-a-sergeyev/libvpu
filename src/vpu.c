...
int VPUClientInit(VPU_CLIENT_TYPE type)
{
    ...
    if (vpu_service_status) {
        ...
        if (fd == -1) {
            VPM_ERROR("VPUClient: failed to open vpu_service\n");
            return -1;
        }
        ret = ioctl(fd, VPU_IOC_SET_CLIENT_TYPE, (RK_U32)type);
        if (ret) {
            VPM_ERROR("VPUClient: ioctl VPU_IOC_SET_CLIENT_TYPE failed ret %d\n", ret);
            return -2;
        }
        return fd;
    } else {
        ...
        if (socket < 0)
        {
            VPM_DEBUG("VPU: socket vpu_server link fail\n");
            return socket;
        }

        VPM_DEBUG("VPU: VPU client %d link success\n", socket);

        chn.client_type = type;

        chn.pid = gettid();

        VPM_DEBUG("VPU: VPU client getpid %d\n", chn.pid);
        ...
        {
            VPM_DEBUG("VPUClientInit success\n");
        }
        else
        {
            VPM_DEBUG("VPUClientInit fail 0\n");
            if (-1 != socket)
            {
            ...
            VPM_DEBUG("VPUClientInit fail 1\n");
        }
...
RK_S32 VPUClientRelease(int socket)
{
    ...
    if (vpu_service_status) {
        ...
        VPM_DEBUG("VPUClientRelease success\n");
...
RK_S32 VPUClientSendReg(int socket, RK_U32 *regs, RK_U32 nregs)
{
    ...
    if (vpu_service_status) {
        ...
        if (ret) {
            VPM_ERROR("VPUClient: ioctl VPU_IOC_SET_REG failed ret %d errno %d %s\n", ret, errno, strerror(errno));
        }
        return ret;
    } else {
        nregs *= sizeof(RK_U32);
        ...
        if (VPU_SUCCESS == ret)
        {
            VPM_DEBUG("VPUClientSendReg VPUSendMsg success\n");
        }
        else
        {
            VPM_DEBUG("VPUClientSendReg VPUSendMsg fail\n");
        }
...
RK_S32 VPUClientWaitResult(int socket, RK_U32 *regs, RK_U32 nregs, VPU_CMD_TYPE *cmd, RK_S32 *len)
{
    ...
    if (vpu_service_status) {
        ...
        if (ret) {
            VPM_ERROR("VPUClient: ioctl VPU_IOC_GET_REG failed ret %d errno %d %s\n", ret, errno, strerror(errno));
        ...
        VPM_DEBUG("VPUClientWaitResult start\n");

        retlen = VPURecvMsg(socket, cmd, regs, nregs, 0);

        if (retlen >= 0)
        {
            VPM_DEBUG("VPUClientWaitResult success\n");
            ret = VPU_SUCCESS;
        }
        else
        {
            VPM_DEBUG("VPUClientWaitResult fail\n");
            ret = VPU_FAILURE;
        }
...
RK_S32 VPUClientGetHwCfg(int socket, RK_U32 *cfg, RK_U32 cfg_size)
{
    ...
    if (vpu_service_status) {
        ...
        if (ret) {
            VPM_ERROR("VPUClient: ioctl VPU_IOC_GET_HW_FUSE_STATUS failed ret %d\n", ret);
        }
        return ret;
    } else {
        RK_S32 ret;
        ...
        if (VPU_SUCCESS == ret)
        {
            VPM_DEBUG("VPUClientGetHwCfg VPUSendMsg success\n");
        }
        else
        {
            VPM_DEBUG("VPUClientGetHwCfg VPUSendMsg fail\n");
            return ret;
        }
        ...
        {
            VPM_DEBUG("VPUClientGetHwCfg VPURecvMsg success\n");
            ret = VPU_SUCCESS;
        }
        else
        {
            VPM_DEBUG("VPUClientGetHwCfg VPURecvMsg fail\n");
            ret = VPU_FAILURE;
        }
