    ...
    do {
        ...
        if (pool) {
            ...
            if (pool) {
                ...
                VPM_DEBUG("VPUMemPoolSet: add pfn %d count %d", pfn, count);
            } else {
...
RK_S32 VPUMemPoolUnset(RK_U32 size, RK_U32 count)
{
    ...
    do {
        ...
        vpu_mem_pool * pool = find_pool_by_pfn(pfn);
        if (pool) {
            VPM_DEBUG("VPUMemPoolUnset: del pfn %d count %d", pfn, count);
            pool->count -= count;
            if (pool->count <= 0) {
...
{
    if (NULL == p) {
        VPM_ERROR("VPUMemMalloc: NULL ptr for handle");
        return INVALID_NULL_PTR;
    }
    *p = NULL;
    if (check_allocator(allocator)) {
        VPM_ERROR("VPUMemMalloc: invalid allocator %p", allocator);
...
{
    if (NULL == p) {
        VPM_ERROR("VPUMemImport: NULL ptr for handle");
        return INVALID_NULL_PTR;
    }
    *p = NULL;
    if (check_allocator(allocator)) {
        VPM_ERROR("VPUMemImport: invalid allocator %p", allocator);
        return INVALID_ALLOCATOR;
    }
...
{
    if (NULL == p) {
        VPM_ERROR("VPUMemFree: NULL ptr for handle");
        return INVALID_NULL_PTR;
    }
    if (check_mem_hnd(p)) {
        VPM_ERROR("VPUMemFree: invalid mem hnd %p", p);
        return INVALID_MEM_HND;
    }
    VPUMemSlot *pSlot = (VPUMemSlot *)p;
    if (check_allocator(pSlot->allocator)) {
        VPM_ERROR("VPUMemFree: invalid allocator %p", pSlot->allocator);
        return INVALID_ALLOCATOR;
    }
...
RK_S32 VPUMemShare(VPUMemHnd *p, RK_S32 *mem_fd)
{
    if (NULL == p || NULL == mem_fd) {
        VPM_ERROR("VPUMemShare: NULL ptr for handle");
        return INVALID_NULL_PTR;
    }
    if (check_mem_hnd(p)) {
        VPM_ERROR("VPUMemShare: invalid mem hnd %p", p);
        return INVALID_MEM_HND;
    }
    VPUMemSlot *pSlot = (VPUMemSlot *)p;
    if (check_allocator(pSlot->allocator)) {
        VPM_ERROR("VPUMemShare: invalid allocator %p", pSlot->allocator);
        return INVALID_ALLOCATOR;
    }
    VPUMemAllocator *pAlloc = (VPUMemAllocator *)pSlot->allocator;
    RK_S32 err;
...
{
    if (NULL == p) {
        VPM_ERROR("VPUMemCache: NULL ptr for handle");
        return INVALID_NULL_PTR;
    }
    if (check_mem_hnd(p)) {
        VPM_ERROR("VPUMemCache: invalid mem hnd %p", p);
        return INVALID_MEM_HND;
    }
    VPUMemSlot *pSlot = (VPUMemSlot *)p;
    if (check_allocator(pSlot->allocator)) {
        VPM_ERROR("VPUMemCache: invalid allocator %p", pSlot->allocator);
        return INVALID_ALLOCATOR;
    }
    VPUMemAllocator *pAlloc = (VPUMemAllocator *)pSlot->allocator;
    RK_S32 err;
